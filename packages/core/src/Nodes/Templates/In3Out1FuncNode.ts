...
export class In3Out1FuncNode<In1, In2, In3, Out1> extends FunctionNode {
  constructor(
    ...
    if (inputValueTypes.length !== 3) {
      throw new Error(
        `inputValueTypes of ${description.typeName}  must have a length of 3, it is instead ${inputValueTypes.length}`
      );
    }
    if (inputNames.length !== 3) {
      throw new Error(
        `inputNames of ${description.typeName}  must have a length of 3, it is instead ${inputNames.length}`
      );
    }
    super(
      description,
