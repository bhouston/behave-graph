    ...
    node.inputSockets.forEach((inputSocket) => {
      ...
      inputSocket.links.forEach((nodeSocketRef) => {
        ...
        if (upstreamNode === undefined) {
          errorList.push(`node ${node.typeName}.${inputSocket.name} has link using invalid nodeid: ${nodeSocketRef.nodeId}`);
          return;
        }

        // check if the socketName is correct
        const outputSocket = upstreamNode.outputSockets.find((socket) => (socket.name === nodeSocketRef.socketName));
        if (outputSocket === undefined) {
          errorList.push(`node ${node.typeName}.${inputSocket.name} has link using a non-existant socket name: `
        ...
        if (inputSocket.valueTypeName !== outputSocket.valueTypeName) {
          errorList.push(`type mismatch between ${node.typeName}.${inputSocket.name} [${inputSocket.valueTypeName}] `
