...
export function vec2Normalize(a: Vec2): Vec2 {
  const invLength = 1.0 / vec2Length(a);
  return vec2Scale(a, invLength);
}
export function vec2Dot(a: Vec2, b: Vec2): number {
  return a.x * b.x + a.y * b.y;
}
export function vec2FromArray(array: Float32Array, offset = 0): Vec2 {
  return new Vec2(array[offset + 0], array[offset + 1]);
}
export function vec2ToArray(a: Vec2, array: Float32Array, offset = 0): void {
  array[offset + 0] = a.x;
  array[offset + 1] = a.y;
}
