...
export function vec3Normalize(a: Vec3): Vec3 {
  const invLength = 1.0 / vec3Length(a);
...
export function vec3FromArray(array: Float32Array, offset = 0): Vec3 {
  return new Vec3(array[offset + 0], array[offset + 1], array[offset + 2]);
}
export function vec3ToArray(a: Vec3, array: Float32Array, offset = 0): void {
  array[offset + 0] = a.x;
  array[offset + 1] = a.y;
  array[offset + 2] = a.z;
}
